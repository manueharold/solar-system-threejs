...
import { currentZoomTarget } from "./path/to/your/initScene.js";
...
export function loadOrbitPlanets(scene, camera, controls) {
  ...
  Object.keys(orbitRadii).forEach((planetName) => {
    ...
    // Use a dashed line material for visual effect.
    ...
    // Create the orbit line and compute dash distances.
    const orbitLine = new THREE.Line(orbitGeometry, orbitMaterial);
    orbitLine.computeLineDistances();
    scene.add(orbitLine);

    // Random starting angle for the planet.
    const initialAngle = Math.random() * Math.PI * 2;

    // Compute orbit speed (using simulationSpeedFactor and orbitalPeriods)
    const orbitSpeed = (2 * Math.PI) / (orbitalPeriods[planetName] * 86400) * simulationSpeedFactor;

    // Set the initial position for the planet
  ...
  // --- Adjust the Camera & Controls for Orbit Mode ---
  ...
  // IMPORTANT: Update the global zoom target to the center of orbit mode
  currentZoomTarget.set(0, 0, 0);
