...
export function loadOrbitPlanets(scene, camera, controls) {
  ...
  Object.keys(orbitRadii).forEach((planetName) => {
    ...
    orbitingPlanets.push({
      ...
      angle: initialAngle,
      speed: orbitSpeed,
      // Add a "paused" flag (false means orbiting is active)
      paused: false
    });
  });

  // --- Adjust the Camera & Controls ---
  // Set the camera position so its distance from (0,0,0) is within the desired zoom limits.
  camera.position.set(0, 10000, 15000); // This positions the camera ~18,027 units from the origin.
  camera.lookAt(0, 0, 0);
  if (controls) {
    controls.target.set(0, 0, 0);
    // Set zoom limits: minimum (zoom in limit) and maximum (zoom out limit) distances.
