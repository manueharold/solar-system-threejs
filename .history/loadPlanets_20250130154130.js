...
export function moveToPlanet(planetName, camera, controls, scene) {
    ...
    // Ensure we don't go inside large planets
    const minZoom = Math.max(planetSize * 0.6, 800);   // Prevents getting too close
    const maxZoom = Math.max(planetSize * 3, 7000);  // Adjusted for large planets
