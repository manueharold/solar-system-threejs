...
export function comparePlanets(newLeftName, newRightName, scene, camera, controls) {
    ...
    // Check if planets have switched sides
    ...
    if (currentComparison.left !== newLeftName || isSwapped) {
        if (currentComparison.leftObject) {
            animateOut(currentComparison.leftObject, 'left', () => {
                // Check if the planet is still in the scene before removing it
                ...
                // Load new left planet and check if it’s already in the scene
                const newLeftPlanet = getPlanetByName(newLeftName);
                if (!newLeftPlanet) {
                    console.error(`Planet "${newLeftName}" not found in scene.`);
                    return;
                }
                // Only add it if not already in the scene
