...
export function moveToPlanet(planetName, camera, controls, scene) {
    ...
    let sidewaysOffset = Math.abs(targetFocus.x - startPos.x) * 1.2; // Ensure clear left/right movement

    // 🎯 Step 1: Move sideways FIRST
    let step1Position = new THREE.Vector3(
        startPos.x + movementDirection * sidewaysOffset, // Strictly left or right
        startPos.y,
        startPos.z
    );

    // 🎯 Step 2: Move forward toward the planet
    let step2Position = new THREE.Vector3(
        step1Position.x, // Keep the same x (no wobble)
        step1Position.y,
        finalTargetPosition.z * 0.8 // Approach the target
    );

    // 🎯 Step 3: Final approach
    let step3Position = finalTargetPosition.clone();
    ...
    gsap.to(progress, {
        t: 1,
        duration: 2.5, // Faster movement
        ease: "power2.out",
        onUpdate: () => {
            if (progress.t < 0.4) {
                camera.position.lerp(step1Position, 0.1); // Sideways movement first
            } else if (progress.t < 0.8) {
                camera.position.lerp(step2Position, 0.1); // Forward movement
            } else {
                camera.position.lerp(step3Position, 0.1); // Final approach
            }

            // 🪐 Show planet info when close
