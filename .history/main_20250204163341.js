...
export function comparePlanets(newLeftName, newRightName, scene, camera, controls) {
  ...
  if (currentComparison.left !== newLeftName || isSwapped) {
    ...
    // The left planet is retained. Remove and re-add to reset its state.
    if (currentComparison.leftObject) {
      if (currentComparison.leftObject.parent) {
        currentComparison.leftObject.parent.remove(currentComparison.leftObject);
      }
      ...
      // Update layout after resetting the retained planet.
      performComparisonLayout(scene, camera, controls);
    }
  }
  ...
  if (currentComparison.right !== newRightName || isSwapped) {
    ...
    // The right planet is retained. Remove and re-add to reset its state.
    if (currentComparison.rightObject) {
      if (currentComparison.rightObject.parent) {
        currentComparison.rightObject.parent.remove(currentComparison.rightObject);
      }
  ...
  // Update layout if both sides are already set.
