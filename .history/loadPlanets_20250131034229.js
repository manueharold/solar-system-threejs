...
export function moveToPlanet(planetName, camera, controls, scene) {
    ...
    // Pause moon orbit if moving to another planet
    moonOrbitPaused = planetName === "moon";

    // Hide all other planets except target
    Object.keys(planets).forEach(name => {
        if (name !== planetName) planets[name].visible = false;
    });
    ...
    gsap.to(camera.position, {
        ...
        onComplete: () => {
            ...
            // Show planets again after movement is complete
            Object.keys(planets).forEach(name => planets[name].visible = true);
        }
    });
