...
export function loadOrbitPlanets(scene, camera, controls) {
    ...
    orbitLoader.load(
        ...
        (gltf) => {
            ...
            // Update the orbit mode UI icon to show it's active
            const orbitIcon = document.getElementById("toggleOrbitMode");
            if (orbitIcon) {
                orbitIcon.classList.add("active");
            }
        },
        undefined,
        (error) => {
            console.error('❌ Failed to load orbit mode model:', error);
            // Optionally remove active state if there's an error
            const orbitIcon = document.getElementById("toggleOrbitMode");
            if (orbitIcon) {
                orbitIcon.classList.remove("active");
            }
        }
    );
}
