...
function createRealisticSun(scene, position, size) {
    ...
    const material = new THREE.MeshBasicMaterial({
      map: sunTexture,
      emissive: 0xffcc00, // Glowing effect
...
export function updatePlanets() {
  for (const planetName in planets) {
    ...
    if (rotationSpeeds[planetName]) {
      // Ensure we rotate the actual mesh, not just the texture
      const mainObject = planet.children.length > 0 ? planet.children[0] : planet;
      mainObject.rotation.y += rotationSpeeds[planetName];
    }
  }
