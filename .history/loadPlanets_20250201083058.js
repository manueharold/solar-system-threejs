...
// ✅ Handle Planet Click
export function onMouseClick(event, setSelectedPlanet) {
    // Raycasting to detect planet click
    const mouse = new THREE.Vector2();
    mouse.x = (event.clientX / window.innerWidth) * 2 - 1;
    mouse.y = -(event.clientY / window.innerHeight) * 2 + 1;

    const raycaster = new THREE.Raycaster();
    raycaster.updateMousePosition(mouse.x, mouse.y);
    ...
    if (intersects.length > 0) {
        ...
        setSelectedPlanet(planetName); // Update selected planet state in React
    }
}
...
export function moveToPlanet(planetName, camera, controls, scene, planets) {
    const targetPlanet = planets[planetName.toLowerCase()]; // Reference the target planet based on name
    if (!targetPlanet) {
        console.error(`Planet ${planetName} not found`);
        return;
    }

    gsap.to(camera.position, { duration: 3, x: targetPlanet.position.x, y: targetPlanet.position.y + 100, z: targetPlanet.position.z + 5000 });
