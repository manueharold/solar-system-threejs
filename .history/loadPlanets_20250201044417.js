...
export function moveToPlanet(planetName, camera, controls, scene) {
    ...
    const baseZoomFactor = 2.5; // Multiplier for zoom level
    let adjustedZoom = Math.max(500, Math.min(planetSize * baseZoomFactor, 5000)); // Dynamic zoom level
    ...
    hidePlanetInfo(); // Hide the panel when moving to a new planet
    ...
    // 🪐 Dynamic camera movement and zoom
    gsap.to(camera.position, {
        ...
        onUpdate: () => {
            ...
            if (!uiShown && distance < adjustedZoom * 1.1) {
                showPlanetInfo(planetName); // Show the planet stats UI
                uiShown = true;
            }
    ...
    // Adjust controls target to the new planet
