...
export function comparePlanets(planet1, planet2, scene, camera, controls) {
  ...
  // Get planet objects
  const planet1Object = scene.getObjectByName(planet1);
  const planet2Object = scene.getObjectByName(planet2);

  if (!planet1Object || !planet2Object) {
      console.error("One or both planets not found in the scene.");
      return;
  }

  // Adjust positions to place them side by side
  planet1Object.position.set(-3, 0, 0); // Move left
  planet2Object.position.set(3, 0, 0);  // Move right

  // Adjust camera position to fit both planets
  camera.position.set(0, 2, 10);
  camera.lookAt(0, 0, 0);

  console.log("Comparison view updated!");
}
