...
import { preloadDefaultPlanets, loadDefaultPlanets } from './loadDefaultPlanets.js';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Preload the default planets early (hidden)
  ...
  setupModeToggles(scene, camera, controls, (isActive) => {
    ...
    if (orbitModeActive) {
      loadOrbitPlanets(scene, camera, controls); // Load solar system model for orbit mode
    } else {
      loadDefaultPlanets(scene, camera, controls); // Show planets and focus on Earth for original mode
    }
  });
  ...
  function animate() {
    ...
    if (orbitModeActive) {
      updateOrbitModeAnimation(deltaTime, scene); // Update orbit animation if in orbit mode
    }
