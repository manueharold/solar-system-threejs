...
// The angular speed for Earth in radians per second would be:
//   (2π / (365 days * 86400 seconds)) * simulationSpeedFactor ≈ 0.1047 rad/s
// Adjust simulationSpeedFactor as desired.
...
export function loadOrbitPlanets(scene, camera, controls) {
  ...
  Object.keys(orbitRadii).forEach((planetName) => {
    ...
    // The ring is initially created in the XY plane, so we rotate it to the XZ plane.
    ...
    // Convert the orbital period from days to seconds: orbitalPeriods[planetName] * 86400.
    // Then calculate angular speed (radians per second) and apply the simulation time warp factor.
