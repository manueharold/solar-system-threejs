...
export function loadOrbitModeModel(scene, camera, controls) {
    ...
    // Optionally, also remove individual planet objects (if not using a group)
        ...
        (gltf) => {
            ...
            if (gltf.animations.length > 0) {
                ...
                gltf.animations.forEach((clip) => {
                    ...
                    action.clampWhenFinished = false; // Prevent snapping at the end
                    action.timeScale = 1;  // Ensure normal playback speed
                    action.play();
                });
