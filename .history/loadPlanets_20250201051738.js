...
export function moveToPlanet(planetName, camera, controls, scene) {
    ...
    // Adjust zoom for all planets
    let adjustedZoom;
    if (planetName === "sun") {
        adjustedZoom = Math.max(planetSize * 8, 150000); // Zoom out even more for Sun
    } else {
        const baseZoomFactor = 8; // Increased zoom out factor for all planets
        adjustedZoom = Math.max(1000, Math.min(planetSize * baseZoomFactor, 10000));
    }

    const targetPosition = new THREE.Vector3(
        targetFocus.x,
        targetFocus.y + planetSize * 0.6, // Keeps a slightly higher view angle
