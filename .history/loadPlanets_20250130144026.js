...
export function moveToPlanet(planetName, camera, controls, scene) {
    ...
    // Get planet's size and adjust focus height
    const box = new THREE.Box3().setFromObject(targetPlanet);
    const planetSize = box.getSize(new THREE.Vector3()).y; // Get height

    const targetPosition = new THREE.Vector3(
        targetPlanet.position.x,
        targetPlanet.position.y + planetSize * 0.6,  // Focus slightly above center
        targetPlanet.position.z + planetSize * 1.2   // Adjust zoom distance
    );
    ...
    gsap.to(camera.position, {
        ...
        duration: 2,
        ease: "power2.out",
        onUpdate: () => camera.lookAt(targetPlanet.position.x, targetPlanet.position.y + planetSize * 0.6, targetPlanet.position.z),
        onComplete: () => {
            console.log(`✅ Arrived at ${planetName}`);
            controls.target.set(targetPlanet.position.x, targetPlanet.position.y + planetSize * 0.6, targetPlanet.position.z);
