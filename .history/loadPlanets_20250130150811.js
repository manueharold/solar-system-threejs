...
export function moveToPlanet(planetName, camera, controls, scene) {
    ...
    const baseZoomFactor = 3; // General zoom level
    let adjustedZoom = planetSize * baseZoomFactor;

    // Ensure zoom stays within min/max limits
    adjustedZoom = Math.max(controls.minDistance, Math.min(adjustedZoom, controls.maxDistance));

    // 🌞 Special case for the Sun: Zoom out more
    if (planetName.toLowerCase() === "sun") {
        adjustedZoom *= 1.8; // Increase zoom distance for the Sun
    }
