...
export function comparePlanets(newLeftName, newRightName, scene, camera, controls) {
  ...
  function addPlanet(side, planetName, callback) {
      ...
      // Hide the planet during animation
  ...
  function showBothPlanets() {
      planetsToShow++;
      if (planetsToShow === 2) {
          console.log("🌍 Showing both planets after animations complete.");
          if (currentComparison.leftObject) currentComparison.leftObject.visible = true;
          if (currentComparison.rightObject) currentComparison.rightObject.visible = true;
      }
  }

  // Hide the unchanged planet while animating the new one
  if (currentComparison.leftObject && leftChanged) {
      console.log(`🚀 Hiding left planet: ${currentComparison.left}`);
      currentComparison.leftObject.visible = false;
      animateOut(currentComparison.leftObject, "left", () => {
          addPlanet("left", newLeftName, (newPlanet) => {
              animateIn(newPlanet, "left", new THREE.Vector3(-1000, 0, 0), showBothPlanets);
          });
      });
  } else {
      showBothPlanets();
  ...
  if (currentComparison.rightObject && rightChanged) {
      console.log(`🚀 Hiding right planet: ${currentComparison.right}`);
      currentComparison.rightObject.visible = false;
      animateOut(currentComparison.rightObject, "right", () => {
          addPlanet("right", newRightName, (newPlanet) => {
              animateIn(newPlanet, "right", new THREE.Vector3(1000, 0, 0), showBothPlanets);
          });
      });
  } else {
      showBothPlanets();
