...
function loadNewSystem() {
    clearScene();  // Clear the previous system

    const loader = new GLTFLoader();
    loader.load('./3d_models/solar_system.glb', (gltf) => {
        const newSolarSystem = gltf.scene;

        // Check if the model is loaded correctly
        console.log('New solar system model loaded:', newSolarSystem);

        // Optionally, scale and position the new model to make sure it's visible
        newSolarSystem.scale.set(100, 100, 100); // Adjust scale if necessary
        newSolarSystem.position.set(0, 0, 0);    // Position it at the center
