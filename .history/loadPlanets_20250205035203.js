 ...
 * Loads Earth first (if needed), then loads the rest concurrently.
 *
...
export async function loadPlanets(scene) {
  ...
  try {
    // Optionally load Earth first if its availability is required for UI ordering
    ...
    // List of planet load promises (excluding Earth and Sun which are already loaded)
    ...
    // Load all remaining planets concurrently
