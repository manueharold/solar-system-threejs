...
export function moveToPlanet(planetName, camera, controls, scene) {
  ...
  // Use planet-specific zoom multipliers.
  // For example, a larger multiplier for Jupiter to ensure we do not end up too close.
  ...
  // Calculate adjusted zoom based on the planet's bounding box size and the multiplier.
  // You can also add min/max clamps here if desired.
  const adjustedZoom = Math.max(1000, planetSize * zoomMultiplier);

  // Calculate a target camera position such that the camera is above and offset from the planet
