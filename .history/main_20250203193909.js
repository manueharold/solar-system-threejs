...
document.body.addEventListener('click', (e) => {
  if (!searchPanel.contains(e.target)) { // If the click is outside of the search panel
    completeSearch(); // Hide the search panel
  }
});

// Optional: Hide the panel when hitting enter after search input
searchInput.addEventListener('keypress', (e) => {
  if (e.key === 'Enter') {
    completeSearch(); // Call the function to hide the search panel after search is complete
  }
});
