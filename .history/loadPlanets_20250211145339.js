...
export function moveToPlanet(planetName, camera, controls, scene, isOrbitModeActive) {
  return new Promise((resolve, reject) => {
    ...
    // ✅ Remove any extra Sun from the scene before moving to the real one
    if (nameLower === "sun") {
      scene.traverse((obj) => {
        if (obj.name === "sun" && obj !== planets["sun"]) {
          scene.remove(obj);
        }
      });

      // Ensure we are moving to the real Sun
      targetPlanet = planets["sun"];
    }

    if (!targetPlanet) {
      console.error(`❌ Real Sun object not found!`);
      return reject(new Error("Real Sun not found in scene!"));
    }
