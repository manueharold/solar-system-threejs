...
export async function loadDefaultPlanets(scene, camera, controls) {
  ...
  scene.traverse((child) => {
    if (
      ...
      (
        (child.name && child.name.toLowerCase().includes("orbit")) ||
      ...
      console.log(`Removed orbit line: ${child.name || "unnamed"}`);
    }
  });

  // Additionally, if the Sun object exists, remove any orbit lines among its children.
  ...
  if (sunObj) {
    sunObj.traverse((child) => {
      if (
        ...
        console.log(`Removed Sun orbit line from Sun: ${child.name || "unnamed"}`);
      }
    });
  }
  ...
  setTimeout(() => {
    scene.traverse((child) => {
      if (
        ...
        console.log(`(Delayed) Removed orbit line: ${child.name || "unnamed"}`);
      }
    });
    ...
    if (newSunObj) {
      newSunObj.traverse((child) => {
        if (
          ...
          console.log(`(Delayed) Removed Sun orbit line: ${child.name || "unnamed"}`);
        }
      });
    }
  }, 200);
