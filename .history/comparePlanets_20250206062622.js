...
export function updateComparisonRotation() {
  // Compare the rotation of left and right planets
  if (currentComparison.leftObject) {
    const leftPlanetName = currentComparison.leftObject.name;
    // Use rotationSpeeds from loadPlanets.js
    const leftRotationSpeed = rotationSpeeds[leftPlanetName] || 0.002;  // Default to base speed if not found
    currentComparison.leftObject.rotation.y += leftRotationSpeed;
  }

  if (currentComparison.rightObject) {
    const rightPlanetName = currentComparison.rightObject.name;
    // Use rotationSpeeds from loadPlanets.js
    const rightRotationSpeed = rotationSpeeds[rightPlanetName] || 0.002;  // Default to base speed if not found
