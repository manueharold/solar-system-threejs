...
export function moveToPlanet(planetName, camera, controls, scene) {
    ...
    // ✅ Fine-tuned zooming for better results
    let adjustedZoom;
    if (planetSize < 1000) {
        adjustedZoom = planetSize * 2.2;  // 🔍 Super close for tiny planets
    } else if (planetSize < 5000) {
        adjustedZoom = planetSize * 2.5;  // 🔍 Closer for medium planets
    } else {
        adjustedZoom = planetSize * 3.5;  // 📏 Further for big planets
    }

    // ✅ Ensure no planet gets too far or too close
    const minZoom = 100;   // Minimum zoom (closer for tiny planets)
    const maxZoom = 6000;  // Maximum zoom (farther for giants)
    adjustedZoom = Math.max(minZoom, Math.min(adjustedZoom, maxZoom));

    // Calculate final camera position
    const targetPosition = new THREE.Vector3(
        targetFocus.x,
        targetFocus.y + planetSize * 0.2,  // Adjust height dynamically
        targetFocus.z + adjustedZoom       // New improved zoom level
    );
    ...
    // ✅ Keep focus on the planet center
