...
export function moveToPlanet(planetName, camera, controls, scene) {
    ...
    // Get planet's size for proper focus
    const box = new THREE.Box3().setFromObject(targetPlanet);
    const planetSize = box.getSize(new THREE.Vector3()).y; // Get height

    // Camera target (slightly above surface)
    const targetFocus = new THREE.Vector3(
        targetPlanet.position.x,
        targetPlanet.position.y + planetSize * 0.6,
        targetPlanet.position.z
    );

    // Target camera position (adjust zoom distance)
    const targetPosition = new THREE.Vector3(
        targetPlanet.position.x,
        targetPlanet.position.y + planetSize * 0.8,
        targetPlanet.position.z + planetSize * 1.5
    );

    controls.enabled = false; // Temporarily disable controls

    // ✅ Move both the camera AND the focus smoothly using GSAP
    ...
    gsap.to(controls.target, {
        ...
        ease: "power2.out",
        onUpdate: () => {
            camera.lookAt(controls.target); // Gradually shift focus
        },
        onComplete: () => {
