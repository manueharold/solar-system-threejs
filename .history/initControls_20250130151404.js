...
export function initControls(camera, renderer) {
    ...
    controls.updateZoomLimits = function (planet) {
        ...
        if (planetName === "sun" || planetName === "jupiter") {
            // 📌 Lock zoom for Sun and Jupiter by setting min = max
            const lockedDistance = 80000; // Fixed distance for large planets
            controls.minDistance = lockedDistance;
            controls.maxDistance = lockedDistance;
            console.log(`🚫 Zoom locked for ${planet.name}`);
        } else {
    ...
    // 🚨 Ensure zoom is locked on Sun at startup
    setTimeout(() => {
