...
/**
 * Switches from Orbit Mode to Original Mode.
 ...
 * - Positions the camera to focus on Earth.
 *
...
export async function loadDefaultPlanets(scene, camera, controls) {
  ...
  // Call loadPlanets (from loadPlanets.js) to load all the default planets (including the Sun)
  ...
  // Attempt to retrieve Earth's object (using either lowercase or uppercase name)
  ...
  if (earth) {
    ...
    // Set a suitable offset so the camera is zoomed out and looking at Earth.
    const offset = 15000; // Adjust this offset value as needed
    camera.position.set(
    ...
    // If OrbitControls is in use, update its target.
    if (controls) {
