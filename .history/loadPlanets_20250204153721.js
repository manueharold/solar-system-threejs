...
/**
 * The unified animation loop that:
 * - Rotates each planet.
 * - Updates the moon's orbit if not paused.
 * - Updates any comparison rotations.
 */
function animateScene() {
  requestAnimationFrame(animateScene);

  // Rotate each planet that has a rotation speed defined
  ...
  // Update any rotation for compared planets
  ...
  // Update the moon's orbit around Earth if not paused
  if (!moonOrbitPaused && planets["moon"] && planets["earth"]) {
    moonOrbitAngle += deltaTime;
    const moonDistance = planetData.moon.distance;
    const { x: earthX, z: earthZ } = planets["earth"].position;
    planets["moon"].position.set(
      earthX + Math.cos(moonOrbitAngle) * moonDistance,
      planets["moon"].position.y,
      earthZ + Math.sin(moonOrbitAngle) * moonDistance
    );
  }
}
...
/**
 * An optional external update function that can be called to update planets.
 * (Note: This logic is largely handled in animateScene.)
 */
export function updatePlanets() {
  // Rotate planets if needed (duplicate logic to animateScene)
  Object.entries(planets).forEach(([name, planet]) => {
    if (rotationSpeeds[name]) {
      planet.rotation.y += rotationSpeeds[name];
    }
  ...
  // Update moon's orbit (if not paused)
  if (!moonOrbitPaused && planets["moon"] && planets["earth"]) {
    const time = Date.now() * 0.0005;
    const moonDistance = planetData.moon.distance;
    const { x: earthX, z: earthZ } = planets["earth"].position;
    planets["moon"].position.set(
      earthX + Math.cos(time) * moonDistance,
      planets["moon"].position.y,
      earthZ + Math.sin(time) * moonDistance
    );
  }
}
