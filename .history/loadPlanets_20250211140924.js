...
import { updateComparisonRotation, currentComparison } from "./comparePlanets.js";
 ...
 * It also resets any compared planets that are currently selected.
 *
 * If the searched planet is the Sun and it isn’t in the scene,
...
export function moveToPlanet(planetName, camera, controls, scene, isOrbitModeActive) {
  return new Promise((resolve, reject) => {
    ...
    // Try to find the target planet in the scene.
    let targetPlanet = scene.getObjectByName(nameLower);
    
    // Special handling for the Sun:
    if (!targetPlanet && nameLower === "sun") {
      // Re-create the Sun if it was removed (e.g., after a comparison).
      targetPlanet = createRealisticSun(scene, [planetData.sun.distance, 0, 0], planetData.sun.scale);
    }
    ...
    // Reset compared planets: remove any currently compared objects.
