...
// ✅ Load planet model with Smooth Entry
function loadPlanetModel(scene, name, modelPath, position, size) {
    ...
    loader.load(modelPath, (gltf) => {
        ...
        // 🌟 Initial scale set to zero (hidden)
        ...
        // 🎬 Animate Entry (Scale & Fade-in)
        gsap.to(planet.scale, { x: size, y: size, z: size, duration: 1.5, ease: "power2.out" });
        gsap.to(planet.traverse((child) => child.isMesh && child.material), { opacity: 1, duration: 1 });

        console.log(`✅ Loaded: ${planet.name}`);
    });
}

// ✅ Smooth Exit Effect (Fade-out & Remove)
function removePlanet(name) {
    ...
    gsap.to(planet.traverse((child) => child.isMesh && child.material), {
        opacity: 0,
        duration: 1,
        onComplete: () => {
            sceneRef.remove(planet);
            delete planets[name];
        }
    });
}
...
// 🚀 Move camera to a planet smoothly
