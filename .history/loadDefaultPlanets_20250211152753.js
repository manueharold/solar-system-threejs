...
export async function loadDefaultPlanets(scene, camera, controls) {
  ...
  if (earth) {
    ...
    // (Assumes that planetData.earth.distance is the intended offset on the x-axis.)
    earth.position.set(planetData.earth.distance, 0, 0);
    // Reset rotation (if needed).
    earth.rotation.set(0, 0, 0);
    // Reset scale based on your configuration.
    earth.scale.set(planetData.earth.scale, planetData.earth.scale, planetData.earth.scale);
    earth.updateMatrixWorld(true);
    console.log("Earth reset to default transformation:", earth.position, earth.scale);
  } else {
