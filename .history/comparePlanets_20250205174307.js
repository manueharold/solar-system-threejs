...
export function comparePlanets(newLeftName, newRightName, scene, camera, controls) {
  ...
  // Helper function to add a planet safely
  function addPlanet(side, planetName) {
      const lowerName = planetName.toLowerCase();
      let newPlanet = getPlanetInstance(planetName);
      if (!newPlanet) return;
  ...
  if (leftChanged && currentComparison.leftObject) {
      animateOut(currentComparison.leftObject, "left", () => {
          ...
          // Ensure the new planet is added
  ...
  // **Step 2: If no planet was previously selected, add them immediately**
