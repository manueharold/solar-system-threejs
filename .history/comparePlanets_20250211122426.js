...
import { rotationSpeeds, planetTemplates, planetData } from "./loadPlanets.js";
 ...
 * When the Sun is one of the compared objects, its effective radius is reduced
 * for layout purposes so that the view isn’t taken over by the Sun.
 *
...
function performComparisonLayout(scene, camera, controls) {
  ...
  const sphereLeft = new THREE.Box3().setFromObject(currentComparison.leftObject)
    .getBoundingSphere(new THREE.Sphere());
  const sphereRight = new THREE.Box3().setFromObject(currentComparison.rightObject)
    .getBoundingSphere(new THREE.Sphere());

  /* 
    For the Sun, the loaded mesh uses a very high scale (see loadPlanets.js),
    which makes its computed bounding sphere enormous. For comparisons we want
    to override that huge value with a normalized one.
    
    Here we use the Sun’s scale from planetData and multiply it by a small factor.
    For example, if planetData.sun.scale is 100000, multiplying by 0.0001 gives 10.
  */
  const SUN_COMPARISON_RADIUS = planetData.sun.scale * 0.0001; // Adjust factor as needed

  // Helper: For the Sun, return the normalized effective radius.
  const getEffectiveRadius = (obj, sphere) => {
    if (obj.name.toLowerCase() === "sun") {
      return SUN_COMPARISON_RADIUS;
    }
    return sphere.radius;
  };

  const leftRadius = getEffectiveRadius(currentComparison.leftObject, sphereLeft);
  const rightRadius = getEffectiveRadius(currentComparison.rightObject, sphereRight);

  // Calculate the midpoint between the two bounding sphere centers.
  const center = sphereLeft.center.clone().add(sphereRight.center).multiplyScalar(0.5);
  
  // When the Sun is present, reduce the layout margin so that the separation isn’t dominated by it.
  const margin =
    (currentComparison.leftObject.name.toLowerCase() === "sun" ||
     currentComparison.rightObject.name.toLowerCase() === "sun")
      ? 200
      : 1000;
  const separation = leftRadius + rightRadius + margin;
  ...
  // Adjust the camera multiplier:
  // If the Sun is present, we use a lower multiplier (e.g. 20) because its effective size is normalized.
  let cameraMultiplier = 50;
  if (
    currentComparison.leftObject.name.toLowerCase() === "sun" ||
    currentComparison.rightObject.name.toLowerCase() === "sun"
  ) {
  ...
  const cameraDistance = separation + Math.max(leftRadius, rightRadius) * cameraMultiplier;
  const targetCameraPos = new THREE.Vector3(center.x, center.y, center.z + cameraDistance);

  // Animate the camera and OrbitControls target.
