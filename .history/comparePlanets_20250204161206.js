...
function performComparisonLayout(scene, camera, controls) {
  ...
  // Adjust the camera.
  // Increase this multiplier to move the camera further back if needed.
...
compareButton.addEventListener('click', () => {
  ...
  if (planet1 === planet2) {
      // Show an error notification if the same planet is selected for both.
  ...
  // Hide all planets except the two being compared
  // Make sure to use lowercase for matching if your mesh names are stored that way.
  ...
  // Automatically close the compare panel after comparing
  if (comparePanel) {
 ...
 * on that specific planet. This is useful when the two planets have different sizes.
 *
...
export function focusComparedPlanet(planetName, camera, controls, scene) {
  ...
  // Compute the bounding sphere for proper framing.
  ...
  // Use a multiplier to determine how close the camera should get.
  const focusMultiplier = 2; // Increase or decrease this value to adjust zoom-in distance.
  const targetDistance = Math.max(planetRadius * focusMultiplier, 1000);
  
  // Compute the new camera position (positioned slightly above and behind the planet).
...
export function hideNonComparedPlanets(scene, keepVisible) {
  scene.traverse((object) => {
    // Check if this object is a Mesh and its name matches one of your planet names.
    // You might have a naming convention for your planet meshes.
    if (object.isMesh && object.name) {
      // If the object's name is not in the keepVisible array, hide it.
...
/**
 * (Optional) Restore visibility for all planets.
 * You can call this function when you exit the comparison view.
 * @param {THREE.Scene} scene - The scene containing the planets.
 */
