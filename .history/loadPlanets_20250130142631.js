...
export function moveToPlanet(planetName, camera, controls, scene) {
    ...
    // ✅ Fix: Hide all other planets except the selected one
    ...
    // ✅ Fix: Ensure the selected planet remains visible
    targetPlanet.visible = true;

    console.log("🎥 Moving camera to:", targetPosition);

    // Target position slightly ahead of the planet
    const finalCameraPosition = new THREE.Vector3(
        targetPosition[0],
        targetPosition[1] + 500, // Slight elevation
        targetPosition[2] + 1500 // Pull back for better view
    );
    ...
    new TWEEN.Tween(camera.position)
        ...
        .onUpdate(() => {
            camera.lookAt(targetPlanet.position); // Ensure camera faces the planet
        })
        .onComplete(() => {
