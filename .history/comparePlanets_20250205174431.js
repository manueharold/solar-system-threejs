...
export function comparePlanets(newLeftName, newRightName, scene, camera, controls) {
  ...
  function addPlanet(side, planetName) {
  ...
  // Step 1: Remove planets only if they changed
  ...
  if (rightChanged && currentComparison.rightObject) {
      ...
      animateOut(currentComparison.rightObject, "right", () => {
          ...
          addPlanet("right", newRightName);
          performComparisonLayout(scene, camera, controls);
      });
  }

  // Step 2: If no planet was previously set, add them immediately
  if (!currentComparison.leftObject) addPlanet("left", newLeftName);
  if (!currentComparison.rightObject) addPlanet("right", newRightName);

  // Step 3: Ensure layout updates happen after planets are in the scene
  if (currentComparison.leftObject && currentComparison.rightObject) {
      console.log("🔄 Performing layout update");
      performComparisonLayout(scene, camera, controls);
  }
}
