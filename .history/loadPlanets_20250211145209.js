...
export function moveToPlanet(planetName, camera, controls, scene, isOrbitModeActive) {
  return new Promise((resolve, reject) => {
    ...
    // If the target is the Sun, ensure the real Sun is in the scene.
    if (nameLower === "sun") {
      // Get the original Sun from our planets object.
      let realSun = planets["sun"];
      
      // Check if the real Sun is in the scene. If not, add it.
      if (!scene.getObjectById(realSun.id)) {
        console.log("Real Sun is not in scene—re-adding it.");
        scene.add(realSun);
      }
      
      // Remove any duplicate suns (for example, the scaled-down comparison Sun)
      scene.traverse((obj) => {
      ...
      // Use the real Sun as our target.
