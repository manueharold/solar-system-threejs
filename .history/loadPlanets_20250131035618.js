...
export function moveToPlanet(planetName, camera, controls, scene) {
    ...
    console.log(`🚀 Moving from: ${lastPlanet} to: ${planetName}`);
    ...
    // Ensure target planet is visible but starts with low opacity
    ...
    gsap.to(camera.position, {
        ...
        onUpdate: () => {
            ...
            // 🌟 Gradually fade in the target planet as we approach
            if (targetPlanet.children[0].material) {
        ...
        onComplete: () => {
            ...
            lastPlanet = planetName; // Update last visited planet

            // 🌌 Gradually fade back in all planets
        // 🌌 Gradually fade out other planets & hide them after animation
Object.keys(planets).forEach(name => {
    if (name !== planetName) {
        gsap.to(planets[name].children[0].material, {
            opacity: 0,
...
// Ensure planets remain hidden during transition
setTimeout(() => {
    Object.keys(planets).forEach(name => {
        if (name !== planetName) {
            planets[name].visible = false;
...
}, 1000); // Delay hiding until after fade-out
