 ...
 * This is the function that runs when Spaceship Mode is activated.
 */
...
function disabledSpaceshipModeHandler(event) {
  ...
  event.stopImmediatePropagation(); // Stop any other click handlers from firing
...
export function setupModeToggles(scene, camera, controls) {
  ...
  orbitButton.addEventListener("click", () => {
    if (!orbitModeActive) {
      ...
      // Disable the Spaceship Mode functionality:
      // Remove the active handler and add the disabled handler.
      spaceshipButton.removeEventListener("click", spaceshipModeHandler);
      spaceshipButton.addEventListener("click", disabledSpaceshipModeHandler);
      ...
      // Re-enable the Spaceship Mode functionality:
      // Remove the disabled handler and add back the active handler.
      spaceshipButton.removeEventListener("click", disabledSpaceshipModeHandler);
      spaceshipButton.addEventListener("click", spaceshipModeHandler);
