...
let moonPosition = new THREE.Vector3(0, 0, 0); // To store the moon's current position when paused
...
function animateScene() {
  ...
  // Rotate planets and update comparisons
  ...
  // Additional updates like rotation comparisons and moon position
  updateComparisonRotation();
}

// --- Handle Planet Movement (Camera Position Change) ---
export function moveToPlanet(planetName, camera, controls, scene) {
  ...
  // If moving away from the Moon, resume its orbit
  if (planetName.toLowerCase() !== "moon" && moonOrbitPaused) {
    moonOrbitPaused = false;
    console.log(`✅ Resuming moon's orbit.`);
  }

  // Save the moon's current position if it's paused
  if (planetName.toLowerCase() === "moon") {
    moonOrbitPaused = true;
    moonPosition.set(planets["moon"].position.x, planets["moon"].position.y, planets["moon"].position.z);
    console.log(`❌ Pausing moon's orbit and saving position.`);
  }
