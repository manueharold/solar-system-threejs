...
// ✅ Load Planet Model
function loadPlanetModel(group, name, modelPath, position, size) {
    loader.load(
        modelPath,
        (gltf) => {
...
export function loadPlanets(planetsGroup) {
    // Reset the group if necessary:
    while(planetsGroup.children.length) {
        planetsGroup.remove(planetsGroup.children[0]);
    }

    sceneRef = planetsGroup;

    // ✅ Correct: Use planetsGroup instead of scene
    loadPlanetModel(planetsGroup, "earth", './3d_models_compressed/earth_draco.glb', [0, 0, 0], 10000);
    loadPlanetModel(planetsGroup, "sun", './3d_models_compressed/sun.glb', [-6000000, 0, 0], 20000);
    loadPlanetModel(planetsGroup, "mercury", './3d_models_compressed/mercury_draco.glb', [-4000000, 0, 0], 4879);
    loadPlanetModel(planetsGroup, "venus", './3d_models_compressed/venus_draco.glb', [-2000000, 0, 0], 8000);
    loadPlanetModel(planetsGroup, "mars", './3d_models_compressed/mars_draco.glb', [2279000, 0, 0], 5200);
    loadPlanetModel(planetsGroup, "jupiter", './3d_models_compressed/jupiter_draco.glb', [7785000, 0, 0], 15000);
    loadPlanetModel(planetsGroup, "saturn", './3d_models_compressed/saturn_draco.glb', [14330000, 0, 0], 13000);
    loadPlanetModel(planetsGroup, "uranus", './3d_models_compressed/uranus_draco.glb', [28770000, 0, 0], 12700);
    loadPlanetModel(planetsGroup, "neptune", './3d_models_compressed/neptune_draco.glb', [45030000, 0, 0], 12000);
    loadPlanetModel(planetsGroup, "moon", './3d_models_compressed/moon_draco.glb', [384400 / 10, 0, 0], 3000);
