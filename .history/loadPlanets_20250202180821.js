...
export function moveToPlanet(planetName, camera, controls, scene) {
  ...
  // Compute the planet's bounding sphere for a more consistent framing
  ...
  // Instead of using the planet size (bounding box length), use the bounding sphere radius.
  // You can adjust these multipliers for each planet if desired.
  const cameraZoomMultipliers = {
      mercury: 8,
      venus: 8,
      earth: 8,
      moon: 8,
      mars: 8,
      jupiter: 15,  // Zoom out more for Jupiter
      saturn: 12,   // Saturn can also be a bit bigger
      uranus: 10,
      neptune: 10,
      sun: 8
  };

  // Get the appropriate multiplier; default to 8 if not found.
  const zoomMultiplier = cameraZoomMultipliers[planetName.toLowerCase()] || 8;

  // Calculate an adjusted distance using the planet's radius.
  // This value defines how far the camera should be from the planet.
  const adjustedDistance = Math.max(planetRadius * zoomMultiplier, 1000);

  // Define a target camera position that is offset upward (y) and along z.
  // Feel free to adjust the 0.6 multiplier for the y offset as needed.
  const targetPosition = new THREE.Vector3(
      targetFocus.x,
      targetFocus.y + planetRadius * 0.6,
      targetFocus.z + adjustedDistance
  );

  // Pause moon orbit when focusing on the moon; resume for other planets.
  ...
  gsap.to(camera.position, {
      ...
      ease: "power2.out",
      onUpdate: () => {
          // When the camera is near the target, show the planet info.
          const distance = camera.position.distanceTo(targetPosition);
          if (!uiShown && distance < adjustedDistance * 1.1) {
