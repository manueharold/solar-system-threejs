...
export function switchToOrbitScene(scene, camera, renderer) {
    // Log the scene children for debugging
    ...
    scene.traverse((child) => {
        if (child instanceof THREE.Mesh || child instanceof THREE.Group) {
            childrenToRemove.push(child); // Mark for removal
        }
    });

    // Remove all the marked children
    childrenToRemove.forEach((child) => {
        scene.remove(child);
    });

    // Log the scene children after removal for debugging
