...
export function moveToPlanet(planetName, camera, controls, scene, isOrbitModeActive) {
  return new Promise((resolve, reject) => {
    let nameLower = planetName.toLowerCase();
    let targetPlanet = scene.getObjectByName(nameLower);
    
    // If the target is the Sun, ensure the proper Sun (and its light) are in the scene.
    if (nameLower === "sun") {
      // Retrieve the original Sun from our planets object.
      let realSun = planets["sun"];
      
      // Reset the Sun’s scale to its original value (from planetData).
      realSun.scale.set(planetData.sun.scale, planetData.sun.scale, planetData.sun.scale);
      
      // If the real Sun isn’t in the scene, re-add it.
      if (!scene.getObjectById(realSun.id)) {
        console.log("Real Sun is not in scene—re-adding it.");
        scene.add(realSun);
      }
      
      // Remove any duplicate (comparison) Sun objects.
      scene.traverse((obj) => {
        if (obj.name === "sun" && obj !== realSun) {
          scene.remove(obj);
        }
      });
      
      // Also ensure that the Sun’s point light is present.
      if (realSunLight && !scene.getObjectById(realSunLight.id)) {
        console.log("Real Sun Light is not in scene—re-adding it.");
        scene.add(realSunLight);
      }
      
      // Set our target to the proper (real) Sun.
      targetPlanet = realSun;
    }
    ...
    // Animate the camera position and OrbitControls target concurrently.
