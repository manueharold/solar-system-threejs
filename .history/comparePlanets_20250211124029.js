...
export function comparePlanets(planet1, planet2, scene, camera, controls) {
  ...
  cleanupOldPlanets().then(() => {
    ...
    // Ensure that we got valid objects.
    if (!currentComparison.leftObject || !currentComparison.rightObject) {
      console.error("One of the compared objects is undefined.");
      return;
    }
    
    // If either planet is the Sun, remove any extra Sun from the scene.
    if (scene && (planet1.toLowerCase() === "sun" || planet2.toLowerCase() === "sun")) {
      scene.traverse((obj) => {
        // Check that obj exists before accessing its properties.
        if (obj && obj.name === "sun" &&
