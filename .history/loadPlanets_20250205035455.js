...
// Use THREE.Clock for smooth delta time calculation
...
function createRealisticSun(scene, position, size) {
  ...
  // Store the sun mesh for later cloning and reference
  planets['sun'] = sunMesh;
...
/**
 * Unified Animation Loop for Planets and the Moon orbit.
 */
function animateScene() {
  ...
  // Rotate Solar System Planets (using deltaTime for framerate independence)
  for (const planetName in planets) {
    const planet = planets[planetName];
    if (rotationSpeeds[planetName]) {
      planet.rotation.y += rotationSpeeds[planetName] * deltaTime;
    }
  }
  ...
  if (!moonOrbitPaused && planets["moon"] && planets["earth"]) {
    // Increase orbit angle (you can adjust the multiplier to change orbit speed)
...
export function moveToPlanet(planetName, camera, controls, scene) {
  ...
  // Animate camera movement with offset using gsap
...
/**
 * (Optional) An external update function in case you want to trigger rotations
 * outside of the main animation loop. Currently, all updates occur in animateScene.
 */
export function updatePlanets() {
  // This function can be removed or repurposed if not needed.
}
