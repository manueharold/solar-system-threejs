...
export function moveToPlanet(planetName, camera, controls, scene) {
    ...
    const midPoint = new THREE.Vector3(
        ...
        (startPos.y + targetPosition.y) / 2 + 2000, // Arc height
    ...
    // 🔄 Fade out all planets except the selected one
    Object.values(planets).forEach((planet) => {
        if (planet !== targetPlanet) {
            gsap.to(planet.children[0].material, { opacity: 0, duration: 0.8, ease: "power2.out", onStart: () => {
                planet.children[0].material.transparent = true;
            }});
        }
    });

    // 🎥 Move the camera along a smooth arc
    gsap.to(camera.position, {
        duration: 2.5,
        ease: "power2.inOut",
        motionPath: {
            path: [startPos, midPoint, targetPosition],
            type: "cubic"
        },
        onUpdate: () => {
        ...
        onComplete: () => {
            ...
            // 🔄 Fade back in the planets after arrival
            Object.values(planets).forEach((planet) => {
                if (planet !== targetPlanet) {
                    gsap.to(planet.children[0].material, { opacity: 1, duration: 1, ease: "power2.out", onComplete: () => {
                        planet.children[0].material.transparent = false;
                    }});
                }
            });
        }
