...
import { SimplexNoise } from "https://cdn.skypack.dev/simplex-noise@2.4.0";  // Check named export
...
function createProceduralPlanet(name, radius, detail, color, position, scene) {
    ...
    const noise = SimplexNoise();  // Now directly using it as a function
    const vertices = geometry.attributes.position;

    for (let i = 0; i < vertices.count; i++) {
        let vertex = new THREE.Vector3().fromBufferAttribute(vertices, i);
        let elevation = noise(vertex.x, vertex.y, vertex.z);  // Call it as a function
