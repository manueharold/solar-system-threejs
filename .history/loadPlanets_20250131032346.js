...
export function moveToPlanet(planetName, camera, controls, scene) {
    ...
    // ✅ Ensure a clean **sideways** arc movement
    let offset = distance * 0.3; // Adjust offset for smoothness
    ...
    const midPoint = new THREE.Vector3(
        (startPos.x + targetPosition.x) / 2 + sideways.x,
        (startPos.y + targetPosition.y) / 2, // Keep height steady
        (startPos.z + targetPosition.z) / 2 + sideways.z
    );
    ...
    gsap.to(progress, {
        ...
        onUpdate: () => {
            ...
            // ✅ Show planet info when camera is actually close
