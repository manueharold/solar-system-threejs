...
export function loadPlanets(scene) {
    ...
    loadPlanetModel(scene, "earth", './3d_models/earth.glb', [0, 0, 0], 3200);
    
    // Place the Sun before Mercury (at a distance of 50 million km)
    loadPlanetModel(scene, "Sun", './3d_models/sun.glb', [-900000, 0, 0], 30000);  // Sun at -50 million km
    
    // Then load the other planets relative to Earth
    loadPlanetModel(scene, "Mercury", './3d_models/mercury.glb', [-600000, 0, 0], 608);  // 58 million km from Earth
    loadPlanetModel(scene, "Venus", './3d_models/venus.glb', [-300000, 0, 0], 1520);  // 108.2 million km from Earth
    loadPlanetModel(scene, "Mars", './3d_models/mars.glb', [227900, 0, 0], 848);  // 227.9 million km from Earth
    loadPlanetModel(scene, "Jupiter", './3d_models/jupiter1.glb', [778500, 0, 0], 17920);  // 778.5 million km from Earth
    loadPlanetModel(scene, "Saturn", './3d_models/saturn.glb', [1433000, 0, 0], 14560);  // 1.433 billion km from Earth
    loadPlanetModel(scene, "Uranus", './3d_models/uranus.glb', [2877000, 0, 0], 6400);  // 2.877 billion km from Earth
    loadPlanetModel(scene, "Neptune", './3d_models/neptune.glb', [4503000, 0, 0], 6000);  // 4.503 billion km from Earth
