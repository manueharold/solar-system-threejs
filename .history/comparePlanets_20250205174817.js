...
export function comparePlanets(newLeftName, newRightName, scene, camera, controls) {
  ...
  function addPlanet(side, planetName, callback) {
      ...
      newPlanet.traverse((child) => {
          if (child.material) child.material.opacity = 1; // Reset opacity
      });
  ...
  function tryLayoutUpdate() {
      planetsAdded++;
      if (planetsAdded >= 2) {
          console.log("🔄 Performing layout update AFTER both planets are in scene");
          performComparisonLayout(scene, camera, controls);
      }
  }

  // **Ensure both planets are added if first-time comparison**
  if (!currentComparison.leftObject && !currentComparison.rightObject) {
      console.log("🆕 First-time planet comparison. Adding both.");
      addPlanet("left", newLeftName, tryLayoutUpdate);
      addPlanet("right", newRightName, tryLayoutUpdate);
      return;
  }

  // Remove and replace left planet if changed
  if (leftChanged && currentComparison.leftObject) {
      console.log(`🚀 Removing left planet: ${currentComparison.left}`);
      animateOut(currentComparison.leftObject, "left", () => {
          console.log(`🗑 Left planet removed: ${currentComparison.left}`);
          scene.remove(currentComparison.leftObject);
          currentComparison.leftObject = null;
          addPlanet("left", newLeftName, tryLayoutUpdate);
      });
  } else {
      tryLayoutUpdate(); // If unchanged, count it as "done"
  }

  // Remove and replace right planet if changed
  if (rightChanged && currentComparison.rightObject) {
      console.log(`🚀 Removing right planet: ${currentComparison.right}`);
      animateOut(currentComparison.rightObject, "right", () => {
          console.log(`🗑 Right planet removed: ${currentComparison.right}`);
