...
export function moveToPlanet(planetName, camera, controls, scene) {
    ...
    let offset = distance * 0.8; // Sideways offset

    // 🎯 Create a simple movement arc
    let fastTravelPoint = new THREE.Vector3(
        startPos.x + movementDirection * offset, // Move purely left or right
        startPos.y,
        startPos.z
    );

    let slowApproachPoint = new THREE.Vector3(
        targetPosition.x - movementDirection * (offset * 0.4), // Still moving in the same direction
        targetPosition.y,
        targetPosition.z - (offset * 0.4)
    );
    ...
    gsap.to(progress, {
        t: 1,
        duration: Math.max(2, distance / 5000000), // Fast movement based on distance
    ...
    gsap.to(controls.target, {
        duration: Math.max(2, distance / 5000000),
        ease: "power2.out",
        x: targetFocus.x,
        y: targetFocus.y,
        z: targetFocus.z,
