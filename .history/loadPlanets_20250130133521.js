import { GLTFLoader } from "https://cdn.skypack.dev/three@0.129.0/examples/jsm/loaders/GLTFLoader.js";
import * as THREE from "https://cdn.skypack.dev/three@0.129.0/build/three.module.js";

// Define rotation period for each planet in Earth days
const rotationPeriods = {
    earth: 1,
    venus: 243,
    mercury: 58.6,
    mars: 1.03,
    jupiter: 0.41,
    saturn: 0.45,
    uranus: 0.72,
    neptune: 0.67
};

// Base rotation speed for Earth (1 day = 0.002)
const baseRotationSpeed = 0.002;

// Calculate rotation speeds for each planet based on its rotation period
const rotationSpeeds = Object.fromEntries(
    Object.entries(rotationPeriods).map(([planet, period]) => [
        planet,
        baseRotationSpeed / period
    ])
);

// Scaling factor (1 AU = 1000 units in the scene)
const AU_SCALE = 15; // Reduced to fit planets better in the scene, can adjust for fitting

// Define realistic distances from the Sun (in AU)
const planetDistances = {
    mercury: 0.39,
    venus: 0.72,
    earth: 1.00,
    mars: 1.52,
    jupiter: 5.20,
    saturn: 9.58,
    uranus: 19.18,
    neptune: 30.07
};

// Function to convert AU to scene units
function convertAUToSceneUnits(AU) {
    return AU * AU_SCALE;
}

// Load planet model with specific position and size
function loadPlanetModel(scene, modelPath, position, fixedSize, rotationSpeed) {
    const loader = new GLTFLoader();
    loader.load(modelPath, (gltf) => {
        const planet = gltf.scene;
        const box = new THREE.Box3().setFromObject(planet);
        const size = box.getSize(new THREE.Vector3());
        const scaleFactor = fixedSize / size.length();
        planet.scale.set(scaleFactor, scaleFactor, scaleFactor);
        planet.position.set(...position);
        scene.add(planet);

        // Set up rotation
        const rotate = () => {
            requestAnimationFrame(rotate);
            planet.rotation.y += rotationSpeed; // Rotation speed
        };
        rotate();
    }, undefined, (error) => {
        console.error(`Error loading ${modelPath.split('/')[2].split('.')[0]} model:`, error);
    });
}

// Initialize the scene
export function initScene() {
    const scene = new THREE.Scene();
    scene.background = null; // Remove the black background

    // Camera setup with adjusted near and far planes
    const camera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 1, 100000000); // Increased far plane
    camera.position.set(0, 1000, 50000);  // Position the camera farther from the center

    // Renderer setup
    const renderer = new THREE.WebGLRenderer({ antialias: true });
    renderer.setPixelRatio(window.devicePixelRatio);
    renderer.setSize(window.innerWidth, window.innerHeight);
    document.body.appendChild(renderer.domElement);

    return { scene, camera, renderer };
}

// Initialize skybox
export function initSkybox(scene) {
    const textureLoader = new THREE.TextureLoader();
    const skyboxTexture = textureLoader.load('textures/panorama_galaxy.png');

    // Create a larger sphere to encompass the entire scene
    const skyboxGeometry = new THREE.SphereGeometry(10000000, 20, 20); // Significantly larger radius
    const skyboxMaterial = new THREE.MeshBasicMaterial({
        map: skyboxTexture,
        side: THREE.BackSide
    });
    const skybox = new THREE.Mesh(skyboxGeometry, skyboxMaterial);
    scene.add(skybox);
}

// Load all the planets with adjusted distances
export function loadPlanets(scene) {
    // Earth
    loadPlanetModel(scene, './3d_models/earth.glb', [convertAUToSceneUnits(planetDistances.earth), 0, 0], 1600, rotationSpeeds.earth);
    
    // Venus
    loadPlanetModel(scene, './3d_models/venus.glb', [convertAUToSceneUnits(planetDistances.venus), 0, 0], 1520, rotationSpeeds.venus);
    
    // Mercury
    loadPlanetModel(scene, './3d_models/mercury.glb', [convertAUToSceneUnits(planetDistances.mercury), 0, 0], 608, rotationSpeeds.mercury);
    
    // Mars
    loadPlanetModel(scene, './3d_models/mars.glb', [convertAUToSceneUnits(planetDistances.mars), 0, 0], 848, rotationSpeeds.mars);
    
    // Jupiter
    loadPlanetModel(scene, './3d_models/jupiter1.glb', [convertAUToSceneUnits(planetDistances.jupiter), 0, 0], 17920, rotationSpeeds.jupiter);
    
    // Saturn
    loadPlanetModel(scene, './3d_models/saturn.glb', [convertAUToSceneUnits(planetDistances.saturn), 0, 0], 14560, rotationSpeeds.saturn);
    
    // Uranus
    loadPlanetModel(scene, './3d_models/uranus.glb', [convertAUToSceneUnits(planetDistances.uranus), 0, 0], 6400, rotationSpeeds.uranus);
    
    // Neptune
    loadPlanetModel(scene, './3d_models/neptune.glb', [convertAUToSceneUnits(planetDistances.neptune), 0, 0], 6000, rotationSpeeds.neptune);
    
    // Sun (No rotation for Sun)
    loadPlanetModel(scene, './3d_models/sun.glb', [0, 0, 0], 30000, 0);
}

// Function to move the camera to the specified planet
export function moveToPlanet(planetName, camera, scene) {
    const planetPositions = {
        mercury: [convertAUToSceneUnits(planetDistances.mercury), 0, 0],
        venus: [convertAUToSceneUnits(planetDistances.venus), 0, 0],
        earth: [convertAUToSceneUnits(planetDistances.earth), 0, 0],
        mars: [convertAUToSceneUnits(planetDistances.mars), 0, 0],
        jupiter: [convertAUToSceneUnits(planetDistances.jupiter), 0, 0],
        saturn: [convertAUToSceneUnits(planetDistances.saturn), 0, 0],
        uranus: [convertAUToSceneUnits(planetDistances.uranus), 0, 0],
        neptune: [convertAUToSceneUnits(planetDistances.neptune), 0, 0],
        sun: [0, 0, 0]
    };

    const targetPosition = planetPositions[planetName.toLowerCase()];

    if (targetPosition) {
        // Animate camera movement
        new THREE.Tween(camera.position)
            .to({ x: targetPosition[0], y: targetPosition[1], z: targetPosition[2] }, 2000)
            .easing(THREE.Easing.Quadratic.Out)
            .start();
    } else {
        console.log("Planet not found!");
    }
}

// Create search button functionality
export function createSearchButton(scene, camera) {
    const searchButton = document.createElement("button");
    searchButton.textContent = "Search Planet";
    document.body.appendChild(searchButton);

    const input = document.createElement("input");
    input.placeholder = "Enter planet name...";
    document.body.appendChild(input);

    searchButton.addEventListener("click", () => {
        const planetName = input.value.trim();
        moveToPlanet(planetName, camera, scene);
    });
}
