...
let lastPlanetName = "earth";

const planetPositions = {
    sun: -5000000, mercury: -1000000, venus: -3000000, earth: 0, mars: 2279000,
    jupiter: 7785000, saturn: 14330000, uranus: 28770000, neptune: 45030000
};
...
export function moveToPlanet(planetName, camera, controls, scene) {
    ...
    let lastPlanet = scene.getObjectByName(lastPlanetName);

    if (!targetPlanet || !lastPlanet) {
    ...
    let adjustedZoom = Math.max(500, Math.min(planetSize * 2.5, 5000));

    let lastPlanetX = planetPositions[lastPlanetName] || 0;
    let targetPlanetX = planetPositions[planetName] || 0;
    let movingRight = targetPlanetX > lastPlanetX;

    let entryOffset = movingRight ? -adjustedZoom : adjustedZoom;

    const startPosition = new THREE.Vector3(
        targetFocus.x + entryOffset,
        targetFocus.y,
        targetFocus.z
    );
    ...
    gsap.to(camera.position, {
        x: targetFocus.x,
        y: targetFocus.y,
        z: targetFocus.z + adjustedZoom,
        duration: 2,
        ease: "power2.out",
        onUpdate: () => camera.lookAt(targetFocus),
        onComplete: () => {
            controls.enabled = true;
            lastPlanetName = planetName;
        }
    });
}
