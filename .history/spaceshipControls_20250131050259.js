export class SpaceshipControls {
    constructor(camera) {
        ...
        this.minY = -200; // Minimum vertical position
        this.maxY = 200;  // Maximum vertical position
    ...
    onKeyChange(event, isPressed) {
        ...
        const actions = {
            ...
            "ArrowUp": () => this.acceleration.y = speed,   // Move Up (local space)
            "ArrowDown": () => this.acceleration.y = -speed,  // Move Down (local space)
    ...
    updateMovement() {
        ...
        if (this.spaceshipMode) {
            ...
            // Clamp vertical position (Y axis)
            this.camera.position.y = THREE.MathUtils.clamp(this.camera.position.y, this.minY, this.maxY);
        }
    }
}
