...
function getPlanetInstance(name) {
  ...
  if (lowerName === "sun") {
    ...
    return template;
  } else if (lowerName === "earth") {
    // Clone the object but keep the same material reference
    const instance = template.clone(true);
    instance.name = lowerName;
    instance.traverse((child) => {
      if (child.isMesh) {
        if (!child.name) child.name = lowerName;
        // Instead of cloning the material, reassign the same material
        // from the original template to preserve texture settings.
        // (Assumes your template's child with the same name exists.)
