...
export const rotationSpeeds = {
  ...
  neptune: rotationSpeeds?.neptune || baseRotationSpeed / 0.67 // fallback if not defined
};
...
export function moveToPlanet(planetName, camera, controls, scene, isOrbitModeActive) {
  return new Promise((resolve, reject) => {
    ...
    // Pause Moon orbit when focusing on a planet (except when focusing on the Moon itself).
    moonOrbitPaused = isOrbitModeActive || (planetName.toLowerCase() !== "moon");
