...
export function showPlanetInfo(planetName) {
    ...
    const planetDetails = planetData[planetName.toLowerCase()];

    if (!planet || !planetDetails) return;

    // Construct the info panel content
    document.getElementById("info").innerHTML = `
        <h2>${planetDetails.name}</h2>
        <p><strong>Fact:</strong> ${planetDetails.fact}</p>
        <p><strong>Diameter:</strong> ${planetDetails.diameter}</p>
        <p><strong>Distance from Sun:</strong> ${planetDetails.distance}</p>
        <p><strong>Orbital Period:</strong> ${planetDetails.orbitalPeriod}</p>
        <p><strong>Moons:</strong> ${planetDetails.moons}</p>
    `;
    document.getElementById("planetInfoPanel").classList.add("active"); // Show info panel
}

// 🌍 Hide Planet Info
export function hidePlanetInfo() {
    document.getElementById("planetInfoPanel").classList.remove("active"); // Hide info panel
}
