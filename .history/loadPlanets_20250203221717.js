...
export async function loadPlanets(scene, camera, controls) {
  sceneRef = scene;
  ...
  try {
    ...
    // Wait for all planets to load before moving the camera
    await Promise.all(loadPlanetPromises);

  } catch (error) {
    console.error("Error loading planets:", error);
  }

  // Move camera to Earth after loading planets
  setTimeout(() => {
    moveToPlanet("earth", camera, controls, scene); // Focus on Earth
    animateScene(); // Start animation after a short delay (if needed)
  }, 3000);
}
...
function animateScene() {
  ...
  // Orbit Moon around Earth (ensure Moon orbits Earth only)
  if (!moonOrbitPaused && planets["moon"] && planets["earth"]) {
    const time = Date.now() * 0.0005;
    const moonDistance = planetData.moon.distance;  // Distance between Earth and Moon
...
export function moveToPlanet(planetName, camera, controls, scene) {
  ...
  // Compute a target camera position
  ...
  if (planetName.toLowerCase() === "moon") {
    moonOrbitPaused = true; // Pause moon orbit when focusing on the moon
  } else {
    moonOrbitPaused = false; // Resume orbit for other planets
  }
