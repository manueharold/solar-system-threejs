 ...
 * The camera follows an arced path (using quadratic Bézier interpolation)
 * to help avoid passing through other planet models.
 *
...
export function moveToPlanet(planetName, camera, controls, scene, isOrbitModeActive) {
  return new Promise((resolve, reject) => {
    ...
    // Compute the planet's bounding sphere for proper framing.
    ...
    // Calculate desired camera distance and an offset above the planet.
    ...
    // Determine the target camera position (above and behind the planet).
    ...
    // Disable controls and hide UI (if needed) before starting the transition.
    controls.enabled = false;
    hidePlanetInfo();

    // --- Compute the arced path for the camera ---
    // Capture the starting position.
    const startPos = camera.position.clone();
    // Compute the midpoint between start and target.
    const midPoint = startPos.clone().add(targetPos).multiplyScalar(0.5);
    // Offset the midpoint upward to create an arc; adjust arcHeight as needed.
    const arcHeight = targetDistance / 2;
    const controlPoint = midPoint.clone().add(new THREE.Vector3(0, arcHeight, 0));

    // Animate the camera position along a quadratic Bézier curve.
    // Quadratic Bézier formula: B(t) = (1-t)² * start + 2*(1-t)*t * control + t² * target, for t in [0,1]
    ...
    gsap.to(tweenObject, {
      ...
      onUpdate: () => {
        ...
        const pos = new THREE.Vector3()
          .copy(startPos).multiplyScalar(oneMinusT * oneMinusT)
          .add(controlPoint.clone().multiplyScalar(2 * oneMinusT * t))
          .add(targetPos.clone().multiplyScalar(t * t));
        camera.position.copy(pos);
      },
      onComplete: () => {
        // Animate the OrbitControls target to move smoothly to the planet's center.
