...
function animatePlanets() {
    function rotate() {
        ...
        animateMoon();  // 🌙 Call the moon orbit function
...
function animateMoon() {
    ...
    moonOrbitAngle += 0.01;  // Adjust speed if needed
    const radius = 384400 / 10;  // Scaled distance from Earth

    // Update Moon's position around Earth
    moon.position.x = earth.position.x + Math.cos(moonOrbitAngle) * radius;
    moon.position.z = earth.position.z + Math.sin(moonOrbitAngle) * radius;
}
...
export function moveToPlanet(planetName, camera, controls, scene) {
    ...
    document.getElementById("searchInput").addEventListener("change", function (event) {
        const selectedPlanet = event.target.value.toLowerCase();
        
        if (planetsList.includes(selectedPlanet.charAt(0).toUpperCase() + selectedPlanet.slice(1))) {
            moveToPlanet(selectedPlanet, camera, controls, scene);
        } else {
            console.error("❌ Planet not found: " + selectedPlanet);
        }
    });
}
