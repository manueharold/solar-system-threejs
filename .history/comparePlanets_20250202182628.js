...
// Global variable to store the current comparison state.
 ...
 * @param {THREE.Camera} camera - The camera (remains fixed).
...
/**
 * Compute target positions for left and right planets and animate them in.
 * This function positions the two compared planets side by side while keeping the camera fixed.
 */
function performComparisonLayout() {
  if (!currentComparison.leftObject || !currentComparison.rightObject) {
    // Both planets need to be available.
    return;
  }
  ...
  // Optionally, update OrbitControls target so that the scene remains centered.
  // Since the camera stays fixed, you might want the controls target to follow the midpoint.
