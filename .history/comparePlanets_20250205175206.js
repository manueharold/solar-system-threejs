...
export function comparePlanets(newLeftName, newRightName, scene, camera, controls) {
  ...
  function addPlanet(side, planetName, callback) {
      ...
      // Set initial visibility to false (prevents flickering)
  ...
  function animatePlanetSwitch(side, oldPlanet, newPlanetName, targetPos) {
      if (oldPlanet) {
          ...
          animateOut(oldPlanet, side, () => {
              ...
              addPlanet(side, newPlanetName, (newPlanet) => {
                  animateIn(newPlanet, side, targetPos, () => {
                      completedAnimations++;
  ...
  if (leftChanged) {
      animatePlanetSwitch("left", currentComparison.leftObject, newLeftName, new THREE.Vector3(-1000, 0, 0));
  } else {
      completedAnimations++;
  }

  if (rightChanged) {
      animatePlanetSwitch("right", currentComparison.rightObject, newRightName, new THREE.Vector3(1000, 0, 0));
  } else {
      completedAnimations++;
  }

  // Ensure the layout update happens **only after both planets are visible**
  setTimeout(() => {
      performComparisonLayout(scene, camera, controls);
  }, 1000);
}
