...
export function moveToPlanet(planetName, camera, controls, scene) {
    ...
    const midPoint1 = new THREE.Vector3(
        (startPos.x + targetPosition.x) / 2 + distance * 0.3, 
        (startPos.y + targetPosition.y) / 2 + distance * 0.5, 
        (startPos.z + targetPosition.z) / 2
    );

    const midPoint2 = new THREE.Vector3(
        (startPos.x + targetPosition.x) / 2 - distance * 0.3, 
        (startPos.y + targetPosition.y) / 2 + distance * 0.7, 
        (startPos.z + targetPosition.z) / 2
    );
    ...
    gsap.to(progress, {
        t: 1,
        duration: 3, // ⏳ Longer for a more cinematic effect
        ease: "power2.inOut",
        onUpdate: () => {
