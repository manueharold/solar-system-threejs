...
export async function loadDefaultPlanets(scene, camera, controls) {
  // Guard: Ensure the scene is defined.
  if (!scene) {
  ...
  // 4. Load default planets using the existing logic from loadPlanets.js
  ...
  if (earth) {
    ...
    // Here we set the camera to be further away from Earth.
    const offsetX = 20000;
    const offsetY = 10000;
    const offsetZ = 20000;
