 ...
 */
// Helper function to search for an object by name (case-insensitive)
function getObjectByNameCaseInsensitive(scene, name) {
  const lowerName = name.toLowerCase();
  let foundObject = null;
  scene.traverse(child => {
    if (child.name && child.name.toLowerCase() === lowerName) {
      foundObject = child;
    }
  });
  return foundObject;
...
export function moveToPlanet(planetName, camera, controls, scene, isOrbitModeActive) {
  return new Promise((resolve, reject) => {
    // Use a case-insensitive lookup so "Sun" and "sun" are treated the same.
    const targetPlanet = getObjectByNameCaseInsensitive(scene, planetName);
    if (!targetPlanet) {
    ...
    // Pause the Moon’s orbit unless we’re focusing on Earth or in Orbit Mode.
    // (This behavior now applies uniformly to any target, including the Sun.)
    moonOrbitPaused = isOrbitModeActive || planetName.toLowerCase() !== "earth";

    // Compute the planet's bounding box and sphere for framing.
    const boundingBox = new THREE.Box3().setFromObject(targetPlanet);
    // In case the object has no geometry (or for non-mesh objects), provide a fallback size.
    if (boundingBox.isEmpty()) {
      // Use a default size or a value stored in userData if available.
    ...
    // Use a default multiplier for most objects.
    let defaultZoomMultiplier = 2;
    // For the Sun, you might want to adjust the multiplier to frame it nicely.
    if (planetName.toLowerCase() === "sun") {
      defaultZoomMultiplier = 1.5;
    }
    const targetDistance = Math.max(planetRadius * defaultZoomMultiplier, 500);
    ...
    // Adjust the target position to avoid collisions with other objects.
    const startPos = camera.position.clone();
    const safeTargetPos = avoidCollisions(startPos, targetPos, scene, planetName.toLowerCase());

    // Disable controls and hide UI before animating.
