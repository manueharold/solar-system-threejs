 ...
 */
function performComparisonLayout(camera, controls) {
...
export function comparePlanets(newLeftName, newRightName, scene, camera, controls) {
  ...
  if (currentComparison.left !== newLeftName || isSwapped) {
    if (currentComparison.leftObject) {
      animateOut(currentComparison.leftObject, "left", () => {
        ...
        leftUpdated = true;
        // Now that the left side is ready, update the layout if the right side is also ready.
        if (rightUpdated || currentComparison.right) {
          performComparisonLayout(camera, controls);
        }
      });
    } else {
      const newLeftPlanet = getPlanetInstance(newLeftName);
  ...
  // (If one or both sides have been updated, their positions/animations will run.)
 ...
 * @param {THREE.Scene} scene - The scene.
 */
export function focusComparedPlanet(planetName, camera, controls) {
