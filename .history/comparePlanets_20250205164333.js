...
import { planetTemplates } from "./loadPlanets.js";

// Global state for current comparisons.
let currentComparison = { left: null, right: null, leftObject: null, rightObject: null };

/**
 * Clones a planet template and ensures unique object naming.
 */
function getPlanetInstance(name) {
  const template = planetTemplates[name.toLowerCase()];
  if (!template) return console.error(`Template for "${name}" not found.`), null;
  const instance = template.clone(true);
  instance.name = name.toLowerCase();
  instance.traverse(child => child.isMesh && (child.name ||= instance.name));
  return instance;
}

/** Removes Earth's moon from its model. */
function removeMoon(planet) {
  planet.name.toLowerCase() === "earth" && planet.traverse(child => child.name?.toLowerCase() === "moon" && child.parent?.remove(child));
}

/**
 * Animates an object offscreen and fades it out.
 */
function animateOut(planet, side, onComplete) {
  gsap.to(planet.position, { x: planet.position.x + (side === "left" ? -2000 : 2000), duration: 1, ease: "power2.in" });
  planet.traverse(child => child.material?.transparent && gsap.to(child.material, { opacity: 0, duration: 1, ease: "power2.in" }));
...
function animateIn(planet, side, targetPos, onComplete) {
  planet.position.set(targetPos.x + (side === "left" ? -2000 : 2000), targetPos.y, targetPos.z);
  planet.traverse(child => child.material?.transparent && (child.material.opacity = 0));
  gsap.to(planet.position, { x: targetPos.x, duration: 1.5, ease: "power2.out" });
  planet.traverse(child => child.material?.transparent && gsap.to(child.material, { opacity: 1, duration: 1.5, ease: "power2.out" }));
