...
export function initControls(camera, renderer) {
    ...
    controls.updateZoomLimits = function (planet) {
        ...
        if (planetName === "sun" || planetName === "jupiter") {
            ...
            controls.enableZoom = true;
            const currentDistance = camera.position.length(); // Keep current zoom level
            controls.minDistance = currentDistance; // Prevent zooming in
            controls.maxDistance = Math.max(150000, currentDistance + 50000); // Allow zooming out
