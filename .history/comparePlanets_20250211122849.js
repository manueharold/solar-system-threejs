 ...
 * For the Sun, reduce its scale to make it fit better in comparisons.
...
function getPlanetInstance(name) {
  ...
  // Ensure all child meshes have a name and are flagged as a planet.
  instance.traverse((child) => {
    if (child.isMesh) {
      if (!child.name) child.name = lowerName;
      // Mark as a planet so we can later hide or show these meshes.
      child.userData.isPlanet = true;
    }
  });
  // **New:** If the planet is the Sun, reduce its scale for comparisons.
  if (lowerName === "sun") {
    // Adjust the factor as needed. Here we reduce the Sun to 5% of its original size.
    instance.scale.multiplyScalar(0.05);
  }
  return instance;
}
 ...
 * Assumes that all planet meshes have `userData.isPlanet` set to true.
 * @param {THREE.Scene} scene - The Three.js scene.
 * @param {string[]} comparedPlanets - Array of planet names (lowercase) that should remain visible.
 */
export function hideNonComparedPlanets(scene, comparedPlanets) {
  scene.traverse((object) => {
    if (object.isMesh && object.userData.isPlanet) {
      // Show the mesh only if its name is in the compared list.
 ...
 * setting up the scene layout, and hiding all other planets.
...
export function comparePlanets(planet1, planet2, scene, camera, controls) {
  ...
  // Cleanup any existing compared planets using Promises for brevity.
  ...
  cleanupOldPlanets().then(() => {
    ...
    // **New:** Hide all other planet meshes except the two selected.
    // (Ensure that when you originally loaded your planets, you flagged them with userData.isPlanet.)
    hideNonComparedPlanets(scene, [planet1.toLowerCase(), planet2.toLowerCase()]);

    // Lay out the compared planets and update the camera.
 ...
 * This can be useful when exiting the comparison mode.
 * @param {THREE.Scene} scene - The Three.js scene.
 */
