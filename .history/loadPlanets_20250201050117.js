...
export function moveToPlanet(planetName, camera, controls, scene) {
    ...
    // Check if controls and controls.target are properly initialized
    if (controls && controls.target) {
        gsap.to(controls.target, {
            ...
            duration: 2,
            ease: "power2.out",
            onUpdate: () => camera.lookAt(controls.target),  // Make sure the camera is always looking at the target
        });
    } else {
        console.warn("Controls or controls.target is undefined");
    }
