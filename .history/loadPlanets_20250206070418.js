...
const orbitAngles = {};
const baseOrbitSpeed = 0.00005;

const orbitSpeeds = {
  mercury: baseOrbitSpeed * Math.pow(planetData.earth.distance / planetData.mercury.distance, 1.5),
  venus:   baseOrbitSpeed * Math.pow(planetData.earth.distance / planetData.venus.distance, 1.5),
  earth:   baseOrbitSpeed,
  mars:    baseOrbitSpeed * Math.pow(planetData.earth.distance / planetData.mars.distance, 1.5),
  jupiter: baseOrbitSpeed * Math.pow(planetData.earth.distance / planetData.jupiter.distance, 1.5),
  saturn:  baseOrbitSpeed * Math.pow(planetData.earth.distance / planetData.saturn.distance, 1.5),
  uranus:  baseOrbitSpeed * Math.pow(planetData.earth.distance / planetData.uranus.distance, 1.5),
  neptune: baseOrbitSpeed * Math.pow(planetData.earth.distance / planetData.neptune.distance, 1.5)
};
...
export async function loadPlanets(scene) {
  ...
  try {
    // Load Earth first for proper ordering
    ...
    // Asynchronously load the remaining planets
    loadPlanetModel(
      scene,
      "mercury",
...
function animateScene() {
  ...
  // Rotate each planet on its own axis (the “spin”)
  for (const planetName in planets) {
    const planet = planets[planetName];
    if (rotationSpeeds[planetName]) {
      planet.rotation.y += rotationSpeeds[planetName]; // self rotation
    }
  }

  // If Orbit Mode is active, update the positions of all planets (except the Sun and Moon)
  if (orbitModeActive) {
    for (const planetName in planets) {
      // Skip the Sun and the Moon (Moon still uses its own orbit around Earth)
      if (planetName === "sun" || planetName === "moon") continue;
      
      // Get the orbital distance (from the Sun) for the planet from planetData
      const distance = planetData[planetName].distance;
      if (!distance) continue; // safety check in case the planetData entry is missing

      // Initialize the orbit angle if not set
      if (typeof orbitAngles[planetName] === 'undefined') {
        // If the planet was originally placed along the positive X axis, start at 0 radians.
        orbitAngles[planetName] = 0;
      }
      
      // Update the orbit angle using the pre–computed speed and the delta time.
      // (deltaTime is computed below from the time elapsed since the last frame)
      orbitAngles[planetName] += orbitSpeeds[planetName] * deltaTime;

      // Update the planet’s position so that it orbits the Sun (assumed at [0,0,0]).
      planets[planetName].position.x = Math.cos(orbitAngles[planetName]) * distance;
      planets[planetName].position.z = Math.sin(orbitAngles[planetName]) * distance;
      // Optionally, you can add a slight Y offset (e.g., a tilt) if desired.
    }
  }

  // Compute delta time for smooth updates
  ...
  // Orbit Moon around Earth using persistent angle if not paused (this logic remains unchanged)
  if (!moonOrbitPaused && planets["moon"] && planets["earth"]) {
    moonOrbitAngle += deltaTime * 0.5; // Adjust speed here
    const moonDistance = planetData.moon.distance;
    const earthPosition = planets["earth"].position;
    planets["moon"].position.x = earthPosition.x + Math.cos(moonOrbitAngle) * moonDistance;
    planets["moon"].position.z = earthPosition.z + Math.sin(moonOrbitAngle) * moonDistance;
    planets["moon"].position.y = earthPosition.y; // Keep same Y-axis as Earth
  }
}
...
export function updatePlanets() {
  ...
  for (const planetName in planets) {
    ...
    if (planetName !== "moon" && rotationSpeeds[planetName]) {
      // Traverse the hierarchy of the planet to find mesh objects
      planet.traverse((child) => {
        if (child.isMesh) {
          // Apply rotation to the mesh
          child.rotation.y += rotationSpeeds[planetName];  // Rotate the mesh only
        }
      });
    }
  }
...
export function setOrbitMode(isActive) {
  orbitModeActive = isActive; // Set the global variable to toggle orbit mode
  console.log(`Orbit mode ${isActive ? 'activated' : 'deactivated'}`);
}
