...
export function moveToPlanet(planetName, camera, controls, scene) {
    ...
    // ✅ Create a **fast, sideways travel path**
    let offset = distance * 0.7; // Sideways offset (larger for longer travel)
    let direction = new THREE.Vector3().subVectors(targetPosition, startPos).normalize();
    let sideways = new THREE.Vector3(-direction.z, 0, direction.x).multiplyScalar(offset); // Sideways motion

    const fastTravelPoint = new THREE.Vector3(
        startPos.x + direction.x * offset + sideways.x,
        startPos.y + direction.y * offset * 0.3, // Keep it mostly sideways
        startPos.z + direction.z * offset + sideways.z
    );

    const slowApproachPoint = new THREE.Vector3(
        targetPosition.x - direction.x * (offset * 0.3),
        targetPosition.y - direction.y * (offset * 0.3),
    ...
    gsap.to(progress, {
        t: 1,
        duration: Math.max(2.5, distance / 4000000), // Fast movement (2-3s max)
        ease: "power2.out", // Faster at start, slower near planet
        onUpdate: () => {
