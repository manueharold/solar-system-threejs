...
export function setupModeToggles(scene, camera, controls) {
  ...
  spaceshipButton.addEventListener("click", () => {
    ...
    if (orbitModeActive) {
      // Option 1: Display an error message and do nothing.
      alert("You cannot use Spaceship Mode while in Orbit Mode");
      // Optionally also log an error:
  ...
  orbitButton.addEventListener("click", () => {
    if (!orbitModeActive) {
      ...
      // Disable the Spaceship Mode button while Orbit Mode is active.
      ...
      // Re-enable the Spaceship Mode button.
