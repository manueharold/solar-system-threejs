...
function loadPlanetModel(scene, modelPath, position, fixedSize, rotationSpeed) {
    ...
    loader.load(modelPath, (gltf) => {
        ...
        planets.push(planet); // Store planet reference
...
// Function to move the camera to the specified planet with zoom effect
export function moveToPlanet(planetName, camera, scene) {
    ...
    if (targetPosition) {
        // Hide all planets except the one we're focusing on
        planets.forEach(planet => {
            if (planet.position.x !== targetPosition[0]) {
                planet.visible = false;
            }
        });

        // Animate camera movement to focus on the selected planet
        new THREE.Tween(camera.position)
            ...
            .onComplete(() => {
                // Zoom in effect: Animate camera's z position for the zoom effect
                new THREE.Tween(camera.position)
                    .to({ x: targetPosition[0], y: targetPosition[1], z: targetPosition[2] - 1000 }, 2000)  // Adjust zoom level by changing z
                    .easing(THREE.Easing.Quadratic.Out)
                    .start();

                // After zoom, show planets again after a delay
                setTimeout(() => {
                    planets.forEach(planet => {
                        planet.visible = true;
                    });
                }, 3000); // Delay for 3 seconds before revealing the planets
            })
            .start();
    } else {
