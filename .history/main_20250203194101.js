...
// 1. Show the search bar when the search icon is clicked
...
// 2. Hide the search bar after the search is complete (on pressing Enter)
searchInput.addEventListener('keypress', (e) => {
  if (e.key === 'Enter') {
    // Hide the search panel when search is completed
    searchPanel.style.display = 'none'; 
  }
});

// 3. Prevent closing when clicking inside the search panel
searchPanel.addEventListener('click', (e) => {
  e.stopPropagation(); // This prevents the click event from propagating to the body or other elements
});

// 4. Close the search panel when clicking outside of it
document.body.addEventListener('click', (e) => {
  if (!searchPanel.contains(e.target) && e.target !== searchButton) { 
    // If the click is outside the search panel and not the search button
