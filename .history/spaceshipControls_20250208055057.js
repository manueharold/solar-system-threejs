export class SpaceshipControls {
    constructor(camera) {
        ...
        this.maxUpSpeed = 1300;  // Limit for upward movement
        this.minDownSpeed = -1300;  // Limit for downward movement
        this.minZoom = 1000;  // Closest zoom distance
        this.maxZoom = 3000;  // Farthest zoom distance
        this.maxSpeed = 2000; // Maximum movement speed
    ...
    onKeyChange(event, isPressed) {
        ...
        const actions = {
            ...
            // Arrow keys (same behavior as WASD for forward/backward/sideways movement)
    ...
    onZoom(event) {
        ...
        const zoomChange = event.deltaY * 0.05; // Adjust zoom sensitivity
        const newZoom = this.camera.position.length() + zoomChange;
    
        // Only apply zoom if the new distance is within limits
        if (newZoom >= this.minZoom && newZoom <= this.maxZoom) {
            this.camera.translateZ(zoomChange);
        }
    }
