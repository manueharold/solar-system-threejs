...
function animateScene(isOrbitModeActive) {
  requestAnimationFrame(() => animateScene(isOrbitModeActive));
  ...
  if (!isOrbitModeActive && !moonOrbitPaused && planets["moon"] && planets["earth"]) {
...
export function moveToPlanet(planetName, camera, controls, scene, isOrbitModeActive) {
  return new Promise((resolve, reject) => {
    ...
    if (!targetPlanet) {
      ...
      reject(new Error(`Planet "${planetName}" not found!`));
      return;
    }
    console.log(`🚀 Moving to: ${planetName}`);

    // Check if Orbit Mode is active or if a planet is being focused.
    if (isOrbitModeActive) {
      moonOrbitPaused = true;  // Pause orbit during Orbit Mode
    } else if (planetName.toLowerCase() !== "moon") {
      moonOrbitPaused = true;  // Pause moon orbit when focusing on another planet
    } else {
      moonOrbitPaused = false;  // If moon is selected, do not pause
    }
    ...
    gsap.to(camera.position, {
      ...
      onComplete: () => {
        ...
        // Resolve the promise when camera movement is done
        resolve();
      }
    });
