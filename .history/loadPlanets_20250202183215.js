...
function createRealisticSun(scene, position, size) {
  ...
  // Adjust the Sun's size using both the actual size and a scale factor
  const sunScaleFactor = planetData.sun.size * planetData.sun.scale * 0.001;  // Adjust multiplier for realistic size
  ...
  // Adjust light intensity to match the new size
  const sunLight = new THREE.PointLight(0xffffff, 5, 0, 2);  // Increased intensity
