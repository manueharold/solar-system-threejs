...
export function moveToPlanet(planetName, camera, controls, scene) {
    ...
    // ✅ Move the camera **closer** for a stronger zoom effect
    const zoomFactor = 800; // Adjust this to control zoom-in distance
    const targetPosition = new THREE.Vector3(
        targetFocus.x,
        targetFocus.y + 500,  // Move up slightly
        targetFocus.z + zoomFactor  // Get much closer to the planet
    );

    controls.enabled = false; // Temporarily disable controls

    // ✅ Smoothly move the camera **closer** to the planet
